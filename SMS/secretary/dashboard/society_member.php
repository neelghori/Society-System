...
<!------------------------------PHP EDIT USER DETAILS START--------------------------------->

<!------------------------------PHP EDIT USER DETAILS END--------------------------------->
...
<form action="user_update.php" method="post" name="member_adding">
  <input type="hidden" name="form_hidden" value="<?php echo "$uid"?>">
      <!-- Modal body -->
      <div class="modal-body">
        <div class="from-group">
        	<label>First name:</label>
        	<input type="text" name="m_firstname" class="form-control" value="<?php echo "$firstname"?>" placeholder="First Name" required>
        	 </div>
        	 <div class="from-group">
        	<label>Last name:</label>
        	<input type="text" name="m_lastname"  class="form-control" value="<?php echo "$lastname"?>" placeholder="Last Name" required>
        	 </div>
            <div class="from-group">
          <label> Email:</label>
          <input type="email" name="email"  class="form-control" value="<?php echo "$emails"?>" placeholder=" Email" required>
           </div>
        	 <div class="from-group">
        	<label>Mobile:</label>
        	<input type="number" name="mobile"  class="form-control"  value="<?php echo "$mobile_no"?>" placeholder="Mobile No" required>
             </div>
             <div class="from-group">
        	<label>Birthday:</label>
            <input type="date" name="birthday_date" class="form-control" value="<?php echo $bdate ?>" required>
             </div>
             <div class="from-group">
        	<label>House No:</label>
        	<input type="text" name="house_no"  class="form-control" value="<?php echo "$house"?>" placeholder="House No" required>
             </div>
             <div class="from-group">
        	<label>Society Name:</label>
        	<input type="text" name="society_name"  class="form-control" value="<?php echo "$society"?>" placeholder="Society Name" required>
             </div>
             <div class="from-group">
            <label>Total Members:</label> 
          <select   name="total_user_memeber" class="form-control">
             <option value="1" <?php if($totalmember == "1"){ echo "selected";}?>>1</option>
              <option value="2" <?php if($totalmember == "2"){ echo "selected";}?>>2</option>
              <option value="3" <?php if($totalmember == "3"){ echo "selected";}?>>3</option>
              <option value="4" <?php if($totalmember == "4"){ echo "selected";}?>>4</option>
              <option value="5" <?php if($totalmember == "5"){ echo "selected";}?>>5</option>
            </select>
             </div>
             <div class="from-group">
        	<label>Entry Date</label>
            <input type="date" name="entry_member_date" class="form-control" value="<?php echo $entrydate ?>" required>
        	 </div>
             <div class="from-group">
        	<label>Photo:</label>
        	<input type="file" name="profile_photo"  class="form-control" value="<?php echo "$user_photo"?>">
             </div>
             
             <div class="from-group">
            <label>Account Type:</label> 
          <select   name="account_type" class="form-control" value="<?php echo "$firstname"?>">
             <option value="1" <?php if( $account_type == "1"){ echo "selected";}?>>Users</option>
              <option value="2" <?php if( $account_type == "2"){ echo "selected";}?>>Admin</option>
            </select>
             </div>
      </div>
...
<!-------------------DISPLAY USER DETAILS IN TABLE START--------------------------->
<div class="res-table">
<table class="table" >
  ...
  echo '<td>'.$row['first_name'].'</td>';
   ...
   echo '<td>'.$row['total_member'].'</td>';
     echo '<td>'.$row['entry_date'].'</td>';
      echo '<td><a href="edit_user_member_detail.php?id='.$row['user_id'].'">
