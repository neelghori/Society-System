          ...
          <div class="col-md-12">
            <div class="card">
              <div class="card-header">
                <h4 class="card-title"> Display Complaint</h4>
              </div>
              <div class="card-body">
...
if(mysqli_num_rows($result)>0){
  ...
  while ($row=mysqli_fetch_array($result)) {
    $id=$row['user_id'];
    $username_display="select house_id from user_entry where user_id='$id'";
   $result1=mysqli_query($conn,$username_display);
     ...
     echo '<td>'.$row['c_details'].'</td>';
      echo '<td>'.$row['c_date'].'</td>';
       echo '<td>'.$row['c_solution'].'</td>';
        echo '<td>'.$row2['house_id'].'</td>';
         echo '<td><a href="complain_edit_secretary.php?id='.$row['complain_id'].'">
